...
df_char_token = pd.read_csv('../people_char_token.csv')
df_radical_token = pd.read_csv('../people_radical_token.csv')
df_tag_token = pd.read_csv('../people_tag_token.csv')
...
out = crf(dense)
model = Model(inputs =[char_in, radical_in], outputs = out)
model = ModelWithCRFLoss(model)
opt = tf.keras.optimizers.Adam(learning_rate=lr_rate)
model.compile(optimizer=opt)

dir_path = 'people_scheme_2'
try:
    os.mkdir(dir_path)
except:
    pass
...
model.fit([X_char, X_radical],y,batch_size= batch_size, epochs=num_epoch, validation_split=0.1, verbose=1,callbacks=[cp_callback])
...
for n in range (1, num_epoch + 1):
    ...
    y_pred_result = model.predict([X_char_te, X_radical_te])[0]
...
df_result.to_csv('people_scheme_2_result.csv')
